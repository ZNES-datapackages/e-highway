...
# if idx in df_2050.index:
#        df[idx].add(row + df_2050.loc[idx])
...
for _, row in df_2030.iterrows():
    if row['from'] in config['countries'] and \
        row['to'] in config['countries']:
...
path = building.write_elements('transshipment.csv',
